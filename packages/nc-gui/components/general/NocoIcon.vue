...
<template>
  <div :style="{ left: `calc(50% - ${size / 2}px)`, top: `-${size / 2}px` }"
    class="color-transition absolute rounded-lg pt-1 pl-1 -ml-1" @click="onClick">
    <div class="relative">
      <img class="hidden dark:block" :width="size" :height="size" alt="NexTable"
        src="~/assets/img/icons/256x256-trans.png" />
      <img class="dark:hidden" :width="size" :height="size" alt="NexTable" src="~/assets/img/icons/256x256.png" />

      <TransitionGroup name="layout" :duration="500">
        <template v-if="animate || ping">
          <div :class="ping ? 'bg-primary bg-opacity-100' : 'animated-bg-gradient'" :style="{
