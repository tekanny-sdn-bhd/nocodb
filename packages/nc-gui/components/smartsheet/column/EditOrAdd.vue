...
const submitBtnLabel = computed(() => {
  const aiAutoSuggestModeLabel = `${t('general.create')} ${activeTabSelectedFields.value.length > 1
...
<template>
  <div v-if="!warningVisible" ref="editOrAddRef" class="overflow-auto nc-scrollbar-md" :class="{
  ...
  }" @keydown="handleEscape" @click.stop @scroll="handleScrollDebounce">
    <a-form v-model="formState" no-style name="column-create-or-edit" layout="vertical" data-testid="add-or-edit-column"
      ...
      <template v-if="!isEdit && !props.fromTableExplorer && (aiAutoSuggestMode || !formState.uidt)">
        <div class="flex flex-col gap-4" :class="{
          ...
          <div class="flex items-center gap-3">
            <div class="flex-1 text-base font-bold text-nc-content-gray">{{ $t('general.new') }} {{ $t('objects.field')
              }}</div>

            <AiToggleButton v-if="isAiFeaturesEnabled" :ai-mode="aiAutoSuggestMode" :ai-loading="aiLoading"
            ...
            <AiWizardTabs v-else v-model:active-tab="activeAiTab" class="!-mx-5">
              <template #AutoSuggestedContent>
                <div class="px-5 pt-4 pb-2">
                  <div v-if="aiError" class="w-full flex items-center gap-3">
                    ...
                    <NcButton size="small" type="text" class="!text-nc-content-brand"
                      @click.stop="handleRefreshOnError">
                      {{ $t('general.refresh') }}
                    </NcButton>
                  </div>
                  ...
                  <div v-else-if="aiAutoSuggestModeStep === 'pick'" class="flex gap-3 items-start">
                      ...
                      <template v-if="activeTabPredictedFields.length">
                        <template v-for="f of activeTabPredictedFields" :key="f.title">
                          <NcTooltip :disabled="selected.length < maxSelectionCount || f.selected">
                            <template #title>
                              <div class="w-[150px]">You can only select {{ maxSelectionCount }} fields to create at a
                                time.</div>
                            </template>

                            <a-tag class="nc-ai-suggested-tag" :class="{
                            ...
                            }" :disabled="selected.length >= maxSelectionCount" @click="onToggleTag(f)">
                              <div class="flex flex-row items-center gap-2 py-[3px] text-small leading-[18px]">
                                <NcCheckbox :checked="f.selected" theme="ai" class="!-mr-0.5"
                                  :disabled="saving || (!f.selected && selected.length >= maxSelectionCount)"
                                  @click.stop="onToggleTag(f, true)" />

                                <component :is="getUIDTIcon(isFormulaPredictionMode ? UITypes.Formula : f.type)"
                                  v-if="isFormulaPredictionMode || f?.type" class="flex-none w-3.5 h-3.5" :class="{
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      " title="Suggest more" placement="top">
                        <NcButton size="xs" class="!px-1" type="text" theme="ai"
                          :loading="aiLoading && calledFunction === 'predictMore'" icon-only :disabled="saving"
                          @click="predictMore">
                          <template #icon>
                      ...
                      <NcTooltip title="Clear all and Re-suggest" placement="top">
                        <NcButton size="xs" class="!px-1" type="text" theme="ai" :disabled="saving"
                          ...
                          <GeneralIcon icon="refresh" class="!text-current" :class="{
              ...
              <template #PromptContent>
                <div class="px-5 pt-4 pb-2 flex flex-col gap-4">
                  <div class="relative">
                    <a-textarea ref="aiPromptInputRef" v-model:value="prompt" :disabled="saving"
                    ...
                    <NcButton size="xs" type="primary" theme="ai" class="!px-1 !absolute bottom-2 right-2" :disabled="!prompt.trim() ||
                      isPredictFromPromptLoading ||
                      (!!prompt.trim() && prompt.trim() === oldPrompt.trim()) ||
                      saving
                      " :loading="isPredictFromPromptLoading" icon-only @click="predictFromPrompt(onSelectedTagClick)">
                      <template #loadingIcon>
            ...
            <div v-if="failedToSaveFields"
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="text-nc-content-gray-muted text-sm">
                  NexTable was unable to add {{ predicted.length }} fields to the table. Please retry adding the fields.
                </div>
              </div>
        ...
        <div v-if="aiAutoSuggestMode" class="sticky -top-5 z-100 bg-white -mx-5 -mt-5 pt-5 px-5" :class="{
          ...
          <a-form-item>
            <div class="flex gap-x-2 justify-end">
              ...
              <!-- Save -->
              <NcButton v-if="aiIntegrationAvailable" v-e="['a:column:ai:add']" html-type="submit" type="primary"
                theme="ai" :loading="saving" :disabled="disableSubmitBtn || saving" size="small"
                :label="submitBtnLabel.label" :loading-label="submitBtnLabel.loadingLabel"
                data-testid="nc-field-modal-submit-btn" @click.prevent="onSubmit">
                <template #icon>
              ...
              <NcButton v-else type="primary" size="small" @click="handleNavigateToIntegrations"> Add AI integration
              </NcButton>
            </div>
          </a-form-item>
        </div>
      ...
      <a-form-item v-if="isFieldsTab" v-bind="validateInfos.title" class="flex">
        <div class="flex flex-grow px-2 py-1 items-center rounded-md bg-gray-100 focus:bg-gray-100 outline-none"
          style="outline-style: solid; outline-width: thin">
          <input ref="antInput" v-model="formState.title" :disabled="readOnly || !isFullUpdateAllowed || isSystem"
            ...
            }" :contenteditable="true" @change="debouncedOnPredictFieldType"
            @input="formState.userHasChangedTitle = true" />
        </div>
      </a-form-item>
      <a-form-item v-if="!props.hideTitle && !isFieldsTab && (aiAutoSuggestMode ? formState.uidt : true)"
        v-bind="validateInfos.title" :required="false" class="!mb-0">
        <a-input ref="antInput" v-model:value="formState.title" class="nc-column-name-input nc-input-shadow !rounded-lg"
          :class="{
            'nc-ai-input': isAiMode,
          }"
          :placeholder="`${$t('objects.field')} ${$t('general.name').toLowerCase()} ${isEdit ? '' : $t('labels.optional')}`"
          :disabled="isKanban || readOnly || !isFullUpdateAllowed" @change="debouncedOnPredictFieldType"
          @input="onAlter(8)" />
      </a-form-item>

      <div class="flex items-center gap-1 empty:hidden">
        <template v-if="!props.hideType && !formState.uidt">
          <SmartsheetColumnUITypesOptionsWithSearch v-if="!(aiAutoSuggestMode && !props.fromTableExplorer)"
            :options="uiTypesOptions" :extra-icons="extraIcons" @selected="onSelectType($event, true)" />
        </template>

        <a-form-item v-else-if="!props.hideType" class="flex-1" @keydown.up.stop="handleResetHoverEffect"
          @keydown.down.stop="handleResetHoverEffect">
          <NcTooltip :disabled="!(!isEdit && formState.uidt && !!formState?.ai_temp_id)">
            <template #title>
              You cannot edit field types of AI-generated fields. Edits can be made after the field is
              created.</template>
            <a-select v-model:open="isColumnTypeOpen" v-model:value="formState.uidt" show-search
              class="nc-column-type-input nc-select-shadow !rounded-lg" :class="{
                'nc-ai-input': isAiMode,
                '!pointer-events-none !cursor-not-allowed': !isEdit && formState.uidt && !!formState?.ai_temp_id,
              }" :disabled="(isEdit && isMetaReadOnly && !readonlyMetaAllowedTypes.includes(formState.uidt)) ||
                isKanban ||
                readOnly ||
              ...
              :filter-option="filterOption" @dropdown-visible-change="onDropdownChange" @change="onSelectType($event)"
              ...
              <a-select-option v-for="opt of uiTypesOptions" :key="opt.name" :value="opt.name"
                ...
                v-bind="validateInfos.uidt" :class="{
                ...
                <NcTooltip class="w-full flex gap-2 items-center justify-between" placement="right"
                  :disabled="!opt?.tooltip" :attrs="{
                    'data-testid': opt.name,
                  }">
                  <template #title> {{ opt?.tooltip }} </template>
                  <div class="flex-1 flex gap-2 items-center max-w-[calc(100%_-_24px)]">
                    <component :is="isAiButtonSelectOption(opt.name) && !isColumnTypeOpen
                    ...
                    <span v-if="opt.isNew || (isAiButtonSelectOption(opt.name) && !isColumnTypeOpen)"
                      class="nc-new-field-badge text-sm text-nc-content-purple-dark bg-purple-50 px-2 rounded-md font-normal">{{
                        $t('general.new') }}</span>
                  </div>
                  <component :is="iconMap.check" v-if="formState.uidt === opt.name" id="nc-selected-item-icon"
                    class="w-4 h-4" :class="{
      ...
      <a-form-item v-if="enableDescription && aiAutoSuggestMode">
        ...
        <a-textarea ref="descInputEl" v-model:value="formState.description" :class="{
          '!min-h-[200px]': props.fromTableExplorer,
          'h-[150px] !min-h-[100px]': !props.fromTableExplorer,
          'nc-ai-input': isAiMode,
        }" class="nc-input-sm nc-input-text-area nc-input-shadow !text-gray-800 px-3 !max-h-[300px]" hide-details
          data-testid="create-field-description-input" :placeholder="$t('msg.info.enterFieldDescription')" />
      </a-form-item>
      ...
      <template v-if="!readOnly && formState.uidt">
        ...
        <SmartsheetColumnLongTextOptions v-if="formState.uidt === UITypes.LongText" v-model="formState"
        ...
        <SmartsheetColumnLinkedToAnotherRecordOptions
          ...
          :key="`${formState.uidt}-${formState.id || 'new'}`" v-model:value="formState" :is-edit="isEdit" />
        ...
        <SmartsheetColumnUserOptions v-if="formState.uidt === UITypes.User" v-model:value="formState"
          :is-edit="isEdit" />
        <SmartsheetColumnSelectOptions
          v-if="formState.uidt === UITypes.SingleSelect || formState.uidt === UITypes.MultiSelect"
          v-model:value="formState" :from-table-explorer="props.fromTableExplorer || false" />
        ...
        <SmartsheetColumnAiButtonOptions
          v-if="formState.uidt === UITypes.Button && formState?.type === ButtonActionsType.Ai" v-model:value="formState"
          :submit-btn-label="submitBtnLabel" :saving="saving" @navigate-to-integrations="handleNavigateToIntegrations"
          @on-submit="onSubmit" />
      </template>
      <template v-if="formState.uidt">
        ...
        <template v-if="!readOnly && isFullUpdateAllowed">
          <div class="nc-column-options-wrapper flex flex-col gap-4">
            ...
            <LazySmartsheetColumnRichLongTextDefaultValue v-if="isTextArea(formState) && formState.meta?.richMode"
              v-model:value="formState" v-model:is-visible-default-value-input="isVisibleDefaultValueInput" />
            <LazySmartsheetColumnDefaultValue v-else-if="
          ...
          <div
            ...
            <div class="nc-more-options flex items-center gap-1 cursor-pointer select-none"
          ...
          <Transition name="layout" mode="out-in">
            <div v-if="advancedOptions" class="overflow-hidden">
              <LazySmartsheetColumnAttachmentOptions v-if="appInfo.ee && isAttachment(formState)"
                v-model:value="formState" />

              <LazySmartsheetColumnAdvancedOptions v-if="formState.uidt !== UITypes.Attachment"
                v-model:value="formState"
        ...
        <a-form-item v-if="enableDescription && !aiAutoSuggestMode" :class="{
          '!pb-4': embedMode,
        }">
          <div class="flex gap-3 text-gray-800 h-7 mb-1 items-center justify-between">
            <span class="text-[13px]">
        ...
        <template v-if="props.fromTableExplorer || aiAutoSuggestMode">
          <a-form-item v-if="!enableDescription" :class="{
            '!pb-4': embedMode,
          }">
        ...
        <template v-else>
          <div
            ...
            <NcButton v-if="!enableDescription && !isSystem" size="small" type="text"
            ...
            <a-form-item v-if="!aiAutoSuggestMode">
              <div class="flex gap-x-2 justify-end" :class="{
                ...
                <!-- Save -->
                <NcButton html-type="submit" type="primary" :theme="isAiMode ? 'ai' : 'default'" :loading="saving"
                  :disabled="!formState.uidt || disableSubmitBtn || saving" size="small" :label="submitBtnLabel.label"
                  :loading-label="submitBtnLabel.loadingLabel" data-testid="nc-field-modal-submit-btn"
