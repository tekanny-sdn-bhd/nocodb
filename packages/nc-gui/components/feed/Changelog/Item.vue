  ...
  return await renderMarkdown(
    truncate.value
      ? Description.replace(/[*_~]|\[.*?\]|<\/?[^>]+(>|$)/g, '')
        ...
        .replace(/^(\*\*)?#?\s*(\p{Emoji})\s*NexTable\s*v[\d.]+(\s*-\s*|\*\*$)/u, '# ')
        .replace(/(!?\(https?:\/\/[^\s)]+\)(?:\]\(https?:\/\/[^\s)]+(?:\s+"[^"]*")?\))?)/g, '')
        .replace('-', '')
        .substring(0, 100)
        .concat('...')
      : Description.replace(/^\[!\[.*?\]\(https?:\/\/.*?\)\]\(https?:\/\/.*?\)/m, '').replace(
        /^(\*\*)?#?\s*(\p{Emoji})\s*NexTable\s*v[\d.]+(\s*-\s*|\*\*$)/u,
        '# ',
      ),
  )
})
    ...
    <div class="flex my-4 px-4 items-center justify-between">
      <div class="flex items-center flex-wrap gap-3">
        <NcBadge :border="false" color="brand"
          class="font-semibold text-[13px] nc-title-badge cursor-pointer whitespace-nowrap">
          {{ Title }}
        </NcBadge>
